...
def test_post_invoked_with_results_matching_search(sqs, test_api_key):
    ...
    def make_assertions():
        ...
        for result in posted_results:
            query = (
                result["webUrl"].replace(
                    "www.theguardian.com", "content.guardianapis.com"
                )
...
def test_valid_response_dict_received(sqs, test_api_key):
    # Define a helper function to keep this test DRY
    def make_assertions():
        ...
        assert messages_in_response == len(fetched_results)
    ...
    with patch("src.lp_graun_sifter.__init__.post", wraps=post) as post_spy:
        response = gather(sqs, queue_url, "new development")
        fetched_results = post_spy.call_args.args[2]
        make_assertions()
